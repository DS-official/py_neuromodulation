...
line_noise_freqs = np.arange(60,125,60)
...
ch1 = all_non_sz_data[0][splice]
ch2 = all_non_sz_data[1][splice]
times = all_non_sz_times[splice]
...
solu =  mne_connectivity.spectral_connectivity_epochs(
data.reshape(1,data.shape[0],data.shape[1]), method='plv', sfreq=raw.info['sfreq'], mode = 'fourier',
            fmin=9, fmax=11, faverage=True).get_data()
